...
import os, sys, time, json, asyncio, traceback, bleTools
...
class BaseAgent(ServiceInterface):
    ...
    def _get_capability(self):
        return bleTools.snake_to_pascal(self._capability.name)
    ...
    async def register(
        self, bus: MessageBus, default: bool = True, path: str = "/org/bluez/agent"
    ):
