               ...
               uint16_t len, uint16_t offset)
{
	...
	printk("hog: read_keyboard_report_reference\n");
               ...
               uint16_t len, uint16_t offset)
{
	...
	printk("hog: read_consumer_report_reference\n");
    return bt_gatt_attr_read(conn, attr, buf, len, offset, &report_reference, sizeof(report_reference));
}
               ...
               uint16_t len, uint16_t offset)
{
	...
	printk("hog: read_mouse_report_reference\n");
    return bt_gatt_attr_read(conn, attr, buf, len, offset, &report_reference, sizeof(report_reference));
}
...
static ssize_t write_protocol_mode(struct bt_conn *conn,
                ...
                uint8_t flags)
{
    printk("hog: write_protocol_mode\n");
...
volatile bool notify_enable;
	...
	ARG_UNUSED(attr);
    printk("hog: input_ccc_changed: ");

	notify_enable = (value == BT_GATT_CCC_NOTIFY);
	printk(notify_enable ? "enabled\n" : "disabled\n");
}
...
void sendReport(uint8_t *buffer)
{
	...
	case 1:
		...
		printk("***Sent Keyboard Report1: x%02x, x%02x result: %d\n", keyboard_report_data[0], keyboard_report_data[1], result);
	...
	case 2:
		...
		printk("***Sent Consumer Report2: x%02x, x%02x result: %d\n", consumer_report_data[0], consumer_report_data[1], result);
	...
	case 3:
		...
		printk("***Sent Mouse Report3: x%02x, x%02x, x%02x, result: %d\n", mouse_report_data[0], mouse_report_data[1], mouse_report_data[2], result);
		
		break;
	default :
		printk("Invalid reportId: %d\n", reportId);
	}
}
