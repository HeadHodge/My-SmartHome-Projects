...
USBacmDevice::USBacmDevice(const usb_config_desc_t *config_desc, USBhost *host)
{
    ...
    for (size_t n = 0; n < config_desc->bNumInterfaces; n++)
    {
        ...
        Serial.printf("add interface type: 0x%X \n", intf->bInterfaceClass);
        
        if (intf->bInterfaceClass == 0x02)
        {
            Serial.printf("add 0x02 usb driver, endpoints: %i \n", intf->bNumEndpoints);
            if (intf->bNumEndpoints != 1) return;
            
            int _offset = 0;
            ep = usb_parse_endpoint_descriptor_by_index(intf, 0, config_desc->wTotalLength, &_offset);
            if(!ep) {
                Serial.printf("error parsing endpoint: '%s' by index, EP num: %d, len: %d \n", intf->bNumEndpoints, config_desc->wTotalLength);
                continue;
            };
            
            //ep_int = ep;
            Serial.printf("EP num: %d, len: %d, \n", intf->bNumEndpoints, config_desc->wTotalLength);
            Serial.printf("address: 0x%02x, EP max size: %d, dir: %s \n", ep->bEndpointAddress, ep->wMaxPacketSize, (ep->bEndpointAddress & 0x80) ? "IN" : "OUT");
            
            esp_err_t err = usb_host_interface_claim(_host->clientHandle(), _host->deviceHandle(), n, 0);
            Serial.printf("interface claim status: %d \n", err);
            //itf_num = 0;
        }
        else if (intf->bInterfaceClass == 0x0a)
        {
            if (intf->bNumEndpoints != 2) return;
            ESP_LOGI("", "EP: CDC 0x0a data.");
            ...
            for (size_t i = 0; i < intf->bNumEndpoints; i++)
            {
                ...
                if(!ep) {
                    Serial.printf("error to parse endpoint by index; EP num: %d/%d, len: %d \n", i + 1, intf->bNumEndpoints, config_desc->wTotalLength);
                    return;
                }
                ...
                Serial.printf("EP num: %d/%d, len: %d \n", i + 1, intf->bNumEndpoints, config_desc->wTotalLength);
            }
...
bool USBacmDevice::init()
{
    ...
    esp_err_t err = usb_host_transfer_alloc(2048, 0, &xfer_write);
    ...
    err = usb_host_transfer_alloc(512, 0, &xfer_read);
...
void USBacmDevice::INDATA()
{
    ...
    memset(xfer_read->data_buffer, 0, 256);
...
void USBacmDevice::OUTDATA(uint8_t *data, size_t len)
{
    ...
    memset(xfer_write->data_buffer, 0, 2048);
