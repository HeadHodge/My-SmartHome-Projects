...
bool enableDrive() {
        ...
        Serial.printf("SD Card Size: %lluMB, sectorSize: %zu, sectorCount: %zu\n", cardSize, sectorSize, numSectors);
...
void formatSD() {
  HWSerial.printf("usbCom::formatSD: sectors: %u\n", 4);

    for(int i=0;i<4;++i){
      //writeSD(i, 0, msc_disk[i], sizeof(msc_disk[i]));
      SD.writeRAW(msc_disk[i], i);
    }
}
